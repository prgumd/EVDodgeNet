...
def MakeImgPairs(ReadPath, WritePath):
    ...
    for dirs in os.listdir(ReadPath):
        ...
        for dir in tqdm(dirs):
            ...
            # if(not os.path.exists(CurrWritePath + os.sep + 'mask')):
            #     os.makedirs(CurrWritePath + os.sep + 'mask')

            for ImgNum in tqdm(range(NumImgsStack, NumFilesInCurrDir+1)):
                # for Img in range(NumImgsStack):
                    # INow = cv2.imread(CurrReadPath + 'events' + os.sep + "event_%d"%(ImgNum-NumImgsStack+Img+1) + '.png')
                    # MaskNow = np.array(cv2.imread(CurrReadPath + 'mask' + os.sep + "mask_%08d"%(ImgNum-NumImgsStack+Img+1) + '.png'))
                ...
                # cv2.imwrite(CurrWritePath + os.sep + 'events' + os.sep +  "event_%d"%(ImgNum-NumImgsStack+1) + '.png', IStacked)
                # cv2.imwrite(CurrWritePath + os.sep + 'mask' + os.sep +  "mask_%08d"%(ImgNum-NumImgsStack+1) + '.png', MaskStacked)
                DirNames.write(CurrWritePath + os.sep + 'events' + os.sep  +  "event_%d"%(ImgNum-NumImgsStack+1) + '.png' + '\n')
                # LabelNames.write(CurrWritePath + os.sep + 'mask' + os.sep  +  "mask_%08d"%(ImgNum-NumImgsStack+1) + '.png' + '\n')
    DirNames.close()
    # LabelNames.close()
...
def main():
    ...
    Parser.add_argument('--ReadPath', default='/media/analogicalnexus/00EA777C1E864BA9/2018/EVDodge/downfacing_dataset',
                        help='Base path of images, Default:/media/analogicalnexus/00EA777C1E864BA9/2018/EVDodge/dataset')
    Parser.add_argument('--WritePath', default='/media/analogicalnexus/00EA777C1E864BA9/2018/EVDodge/downfacing_processed',
