...
def MakeImgPairs(ReadPath, WritePath):
    ...
    LabelNames = open(WritePath + os.sep + 'Labels.txt', 'w')
    for dirs in next(os.walk(ReadPath))[1]:
        NumImgsStack = 2 # How many images to stack?
        ImgNumberStartOffset = 1 # Image number starts from this number
        for dir in tqdm(dirs):
            ...
            if(not os.path.exists(CurrWritePath + os.sep + 'masks')):
                os.makedirs(CurrWritePath + os.sep + 'masks')

            for ImgNum in tqdm(range(0, NumFilesInCurrDir-NumImgsStack+1)):
                for Img in range(NumImgsStack):
                    INow = cv2.imread(CurrReadPath + 'events' + os.sep + "event_%d"%(ImgNum+Img+ImgNumberStartOffset) + '.png')
                    # MaskNow = np.array(cv2.imread(CurrReadPath + 'mask' + os.sep + "mask_%08d"%(ImgNum+Img+ImgNumberStartOffset) + '.png'))
                ...
                cv2.imwrite(CurrWritePath + os.sep + 'events' + os.sep +  "event_%d"%(ImgNum+ImgNumberStartOffset) + '.png', IStacked)
                # cv2.imwrite(CurrWritePath + os.sep + 'masks' + os.sep +  "mask_%d"%(ImgNum+ImgNumberStartOffset) + '.png', MaskStacked)
                DirNames.write(CurrWritePath + os.sep + 'events' + os.sep  +  "event_%d"%(ImgNum+ImgNumberStartOffset) + '.png' + '\n')
                # LabelNames.write(CurrWritePath + os.sep + 'masks' + os.sep  +  "mask_%d"%(ImgNum+ImgNumberStartOffset) + '.png' + '\n')
    DirNames.close()
    LabelNames.close()
...
def main():
    ...
    Parser.add_argument('--ReadPath', default='/media/nitin/Research/EVDodge/DatasetChethanEvents/datasetRaw',
                        help='Base path of images, Default:/media/analogicalnexus/00EA777C1E864BA9/2018/EVDodge/dataset')
    Parser.add_argument('--WritePath', default='/media/nitin/Research/EVDodge/DatasetChethanEvents/Deblurred1',
