...
parser.add_argument("--groundtruth_topic", default="/dvs/pose", help="Ground truth topic")
...
def get_depth_filename(i):
    return "depthmaps/frame_" + str(i).zfill(8)  + ".png"
...
with rosbag.Bag(args.bag, 'r') as bag:
    ...
    for topic, msg, t in bag.read_messages():
        ...
        # ground truth
        if topic == args.groundtruth_topic:
        ...
        #         cv_image_array = np.array(cv_image, dtype = np.dtype('f8'))
        #         cv_image = cv2.normalize(cv_image_array, cv_image_array, 0, 255, cv2.NORM_MINMAX)
        #     except CvBridgeError, e:
        #         print e

        #     depthmaps_file.write(timestamp_str(msg.header.stamp - reset_time) + " ")
        #     depthmaps_file.write(get_filename(depthmap_index) + "\n")

        #     cv2.imwrite(get_filename(depthmap_index), cv_image)
