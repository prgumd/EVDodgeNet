...
def SetupAll(ReadPath):
    ...
    # Image Input Shape
    PatchSize = np.array([180, 190, 3])
    ImageSize = np.array([180, 190, 3])
...
def TestOperation(PatchPH, PatchSize, ModelPath, ReadPath, WritePath, TrainNames, NumTrainSamples, NumImgsStack):
    """
    Inputs: 
    ...
    prImg = EVDeblurNet(PatchPH, PatchSize, 1)
    
    # Image Idx starts from this number
    ImgStartNum = 0
    ...
    with tf.Session() as sess:
        ...
        for dirs in tqdm(next(os.walk(ReadPath))[1]):
            CurrReadPath = ReadPath + os.sep + dirs + os.sep
            ...
            for ImgNum in tqdm(range(0, NumFilesInCurrDir-NumImgsStack)):
                for StackNum in range(0, NumImgsStack):
                    INow = cv2.imread(CurrReadPath + os.sep + "event_%d"%(ImgNum+StackNum+ImgStartNum) + '.png')
                    ...
                    prImgVal = sess.run(prImg, FeedDict)
                    
                    prImgVal = prImgVal[0]
                    # Remap to [0,255] range
                    prImgVal = np.uint8(remap(prImgVal, 0.0, 255.0, np.amin(prImgVal), np.amax(prImgVal)))
                    # Crop out junk pixels as they are appended in top left corner due to padding
                    prImgVal = prImgVal[-PatchSize[0]:, -PatchSize[1]:, :]
                    if StackNum == 0:
                        IStacked = prImgVal
                    else:
                        IStacked = np.hstack((IStacked, prImgVal))
                # Write Image to file
                cv2.imwrite(CurrWritePath  + os.sep +  "event_%d"%(ImgNum+1) + '.png', IStacked)
...
def main():
    ...
    Parser.add_argument('--ModelPath', dest='ModelPath', default='/media/nitin/Research/EVDodge/CheckpointsDeblurNetLR1e-4GradAndPhoto/399model.ckpt',\
                                                         help='Path to load model from, Default:ModelPath')
    Parser.add_argument('--ReadPath', dest='ReadPath', default='/media/nitin/Research/EVDodge/DatasetChethanEvents/processed',\
                                                                             help='Path to load images from, Default:ReadPath')
    Parser.add_argument('--WritePath', dest='WritePath', default='/media/nitin/Research/EVDodge/DatasetChethanEvents/Deblurred',\
    ...
    PatchPH = tf.placeholder(tf.float32, shape=(1, PatchSize[0], PatchSize[1], PatchSize[2]), name='Input')

    TestOperation(PatchPH, PatchSize, ModelPath, ReadPath, WritePath, TrainNames, NumTrainSamples, NumImgsStack)
