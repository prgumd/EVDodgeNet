...
def MakeImgPairs(ReadPath, WritePath):
    ...
    Labels = open(WritePath + os.sep + 'Masks.txt', 'w')
        ...
        for dir in tqdm(dirs):
            ...
            for ImgNum in tqdm(range(NumImgsStack, NumFilesInCurrDir+1)):
                for Img in range(NumImgsStack):
                    ...
                    MaskNow = np.array(cv2.imread(CurrReadPath + 'masks' + os.sep + "mask_%08d"%(ImgNum-NumImgsStack+Img+1) + '.png'))
                ...
                cv2.imwrite(CurrWritePath + os.sep + 'masks' + os.sep +  "mask_%08d"%(ImgNum-NumImgsStack+1) + '.png', MaskStacked)
                DirNames.write(CurrWritePath + os.sep + 'events' + os.sep  +  "event_%d"%(ImgNum-NumImgsStack+1) + '.png' + '\n')
                Labels.write(CurrWritePath + os.sep + 'masks' + os.sep  +  "mask_%08d"%(ImgNum-NumImgsStack+1) + '.png' + '\n')
    DirNames.close()
    Labels.close()
