...
def EVHomographyNetUnsupSmall2(Img, ImageSize, MiniBatchSize):
    """
    Inputs: 
    ...
    conv1 = tf.layers.conv2d(inputs=Img, filters=16, kernel_size=[3, 3], strides=(2, 2), padding="same", activation=None, name='conv1T')
    ...
    conv2 = tf.layers.conv2d(inputs=bn1, filters=32, kernel_size=[5, 5], strides=(2, 2), padding="same", activation=None, name='conv2T')
    ...
    conv3 = tf.layers.conv2d(inputs=bn2, filters=64, kernel_size=[7, 7], strides=(2, 2), padding="same", activation=None, name='conv3T')
    ...
    conv4 = tf.layers.conv2d(inputs=bn3, filters=64, kernel_size=[7, 7], strides=(2, 2), padding="same", activation=None, name='conv4T')
    ...
    # conv5 output is of size M/16 x N/16 x 128
    conv5 = tf.layers.conv2d(inputs=bn4, filters=64, kernel_size=[7, 7], strides=(2, 2), padding="same", activation=None, name='conv5T')

    # bn5 output is of size M/16 x N/16 x 128
    bn5 = tf.layers.batch_normalization(conv5, name='bn5T')

    # bn5 output is of size M/16 x N/16 x 128
    bn5 = tf.nn.relu(bn5, name='relu5T')

    # flat is of size 1 x M/16*N/16*128
    flat = tf.reshape(bn5, [-1, ImageSize[0]*ImageSize[1]*64/(32*32)], name='flat')

    # flatdrop is a dropout layer
    flatdrop = tf.layers.dropout(flat, rate=0.75)

    # fc1 
    fc1 = tf.layers.dense(flatdrop, units=256, activation=None)
