...
def MakeImgPairs(ReadPath, WritePath):
    ...
    for dirs in os.listdir(ReadPath):
        NumImgsStack = 1 # How many images to stack?
        # for dir in tqdm(dirs):
        CurrReadPath = ReadPath + os.sep + dirs + os.sep
        NumFilesInCurrDir = len(glob.glob(CurrReadPath + 'images' + os.sep + '*.png'))
        I = cv2.imread(CurrReadPath + 'images' + os.sep + "frame_%08d"%1 + '.png')
        ImageSize = np.shape(I)
        CurrWritePath = WritePath + os.sep + dirs
        if(not os.path.exists(CurrWritePath + os.sep + 'images')):
            os.makedirs(CurrWritePath + os.sep + 'images')
        ...
        for ImgNum in tqdm(range(NumImgsStack, NumFilesInCurrDir+1)):
            for Img in range(NumImgsStack):
                INow = cv2.imread(CurrReadPath + 'images' + os.sep + "frame_%08d"%(ImgNum-NumImgsStack+Img+1) + '.png')
            ...
            # cv2.imwrite(CurrWritePath + os.sep + 'images' + os.sep +  "frame_%08d"%(ImgNum-NumImgsStack+1) + '.png', IStacked)
            # cv2.imwrite(CurrWritePath + os.sep + 'label' + os.sep +  "label_%08d"%(ImgNum-NumImgsStack+1) + '.png', MaskStacked)
            DirNames.write(CurrWritePath + os.sep + 'images' + os.sep  +  "frame_%08d"%(ImgNum-NumImgsStack+1) + '.png' + '\n')
